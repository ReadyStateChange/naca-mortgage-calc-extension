...
describe("validatePropertyTax", () => {
  ...
  it("rejects tax below minimum", () => {
    ...
    expect(result.kind).toBe("failure");
    expect(result.field).toBe("tax");
    expect(result.message).toBe("Invalid property tax rate");
  });
  ...
  it("rejects non-numeric tax", () => {
    ...
    expect(result.kind).toBe("failure");
    expect(result.field).toBe("tax");
    expect(result.message).toBe("Property tax must be a number");
  });
...
describe("validateCalculatorInput", () => {
  ...
  it("returns first validation error (fail-fast)", () => {
    ...
    expect(result.kind).toBe("failure");
    expect(result.field).toBe("price"); // first field validated
  });
  ...
  it("identifies which field has the error", () => {
    ...
    // fail-fast: returns first error (price comes before rate)
    expect(result.kind).toBe("failure");
    expect(result.field).toBe("price");
  });
