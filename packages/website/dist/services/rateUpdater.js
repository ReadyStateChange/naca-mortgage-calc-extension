...
import { decodeNacaMortgageRates, NacaMortgageRatesEquivalence, } from "../schemas/external/rates";
const ONE_DAY_IN_MS = 24 * 60 * 60 * 1000;
function ratesMatch(latestDbRates, ratesFromWebsite) {
    if (!latestDbRates)
        return false;
    console.log("Testing if rates have changed");
    return NacaMortgageRatesEquivalence(latestDbRates, ratesFromWebsite);
}
export async function saveRatesIfNeeded(ratesFromWebsite) {
    const latestResult = await pool.query("SELECT * FROM naca_mortgage_rates ORDER BY created_at DESC LIMIT 1");
    ...
    if (parsedRatesFromDb.created_at) {
        ...
        if (now.getTime() - lastCreatedAt.getTime() < ONE_DAY_IN_MS) {
            ...
            return { status: "already_saved", lastCreatedAt };
        }
    }
    const insertResult = await pool.query(`INSERT INTO naca_mortgage_rates
